...
import {StyleSheet, Text, View, Image, ScrollView,TouchableOpacity,Modal,Platform} from 'react-native';
  ...
  const id1= await AsyncStorage.getItem('id')
    ...
    if(response.sucecess){
     ...
     await response.data?.map((item,index)=>{
      ...
      if(moment(item.createdAt).format('DD-MM-YYYY')>=moment(new Date().setDate(new Date().getDate() - 4)).format('DD-MM-YYYY') && moment(item.createdAt).format('DD-MM-YYYY')<=moment(new Date()).format('DD-MM-YYYY')){
        if(item.status==5){
          item?.cart_items?.map((item1,index1)=>{
            graph.push(item1.item_total)
            console.log('Get ITme::',item1.item_total)
      ...
      console.log("Get Date:::::",moment(item.createdAt).format('DD-MM-YYYY'),new Date().setDate(new Date().getDate() - 4))
      if(moment(item.createdAt).format('DD-MM-YYYY')==moment(new Date().setDate(new Date().getDate() - 1)).format('DD-MM-YYYY')){
...
const onDateChange=async(dates)=>{
  ...
  const id1= await AsyncStorage.getItem('id')
    ...
    if(response.sucecess){
      ...
      let yesTotal=0
     await response.data?.map((item,index)=>{
      console.log("Get Date:::::",moment(item.createdAt).format('DD-MM-YYYY'),moment(dates).format('DD-MM-YYYY'))
       if(moment(item.createdAt).format('DD-MM-YYYY')==moment(dates).format('DD-MM-YYYY')){
      ...
      console.log("Get Date:::::",moment(item.createdAt).format('DD-MM-YYYY')>=moment(new Date().setDate(new Date(dates).getDate() - 4)).format('DD-MM-YYYY') && moment(item.createdAt).format('DD-MM-YYYY')<=moment(new Date(dates)).format('DD-MM-YYYY'))
      if(moment(item.createdAt).format('DD-MM-YYYY')>=moment(new Date().setDate(new Date(dates).getDate() - 4)).format('DD-MM-YYYY') && moment(item.createdAt).format('DD-MM-YYYY')<=moment(new Date(dates)).format('DD-MM-YYYY')){
         if(item.status==5){
         item?.cart_items?.map((item1,index1)=>{
      ...
      if(moment(item.createdAt).format('DD-MM-YYYY')==moment(new Date().setDate(new Date(dates).getDate() - 1)).format('DD-MM-YYYY')){
        console.log('Get ITme:: YesterDaY',item?.status)
        if(item.status==5){
...
<View style={styles.modalView}>
                ...
                <View style={styles.menu}>
                  ...
                  <TouchableOpacity
                    style={{flexDirection: 'row', alignItems: 'center'}} onPress={()=>Logout()}>
                    <Image
                      source={logout}
